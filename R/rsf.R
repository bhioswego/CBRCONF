...
rsf <- function(dataset, n.folds = 10, n.tree = 500, blocksize = 1000, Cores = 7) {
  ...
  for(i in 1:n.folds) {
    ...
    training_model <- ranger(Surv(time = time, event = status) ~ ., data = training_fold, splitrule = "logrank", importance = "impurity")

    testing_model <- ranger(Surv(time = time, event = status) ~ ., data = testing_fold,  splitrule = "logrank", importance = "impurity")

    testing_concordance <- 1 - testing_model$prediction.error
