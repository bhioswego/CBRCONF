#' Case Based Reasoning with Confidence-Based Retrieval for Survival
#'
...
#' @param Risk_Group_Num The number of risk groups to create
...
CBRCon_WithTrainingResults <- function(dataset, Num = 10, Percent = 0.5, Cores = 2, blocksize = 4000, Risk_Group_Num = 3, n.folds = 10, KM.Plot = TRUE) {
  set.seed(25)
  ...
  risk_groups$`Risk Group` <- ile(prognostic_scores$`prognostic score`, Risk_Group_Num)
  ...
  training_number_retrieved <- data.frame(matrix(nrow = nrow(sig_feature_data), ncol = 1))
  colnames(training_number_retrieved)[1] <- "Number of Retrieved Neighbors"
  rownames(training_number_retrieved) <- rownames(sig_feature_data)

  testing_sample_information <- list()
  training_sample_information <- list()
  ...
  for(i in 1:n.folds) {
    ...
    training_fold_case_information <- list()
    # Extracting testing and training fold
    ...
    training_neighbor_times <- data.frame(matrix(nrow = nrow(training_fold), ncol = 4))
    ...
    colnames(training_neighbor_times)[3] <- "prognostic scores"
    colnames(training_neighbor_times)[4] <- "risk group"
    ...
    training_distance <- as.matrix(dist(training_features))
    ...
    for(k in 1:nrow(training_features)) {
      training_query_case <- rownames(training_features)[k]

      # Creating lists to store survival times and prognostic scores for each query case
      training_retrieved_neighbors <- list()
      training_survival_times_for_case <- list()
      training_prognostic_scores_for_case <- list()
      training_risk_groups_for_case <- list()
      training_single_case_information <- list(training_retrieved_neighbors, training_survival_times_for_case, training_prognostic_scores_for_case, training_risk_groups_for_case)
      names(training_single_case_information) <- c("retrieved neighbors", "survival times", "prognostic scores", "risk groups")

      training_ordered_neighbors <- order(training_distance[k, ])
      training_name_of_neighbors <- colnames(training_distance)[training_ordered_neighbors]
      training_current_confidence <- 0


      for(n in 1:length(training_name_of_neighbors)) {
        training_neighbor <- training_name_of_neighbors[n]
        confidence <- confidence_weights_average[which(rownames(confidence_weights_average) == training_neighbor),]
        training_survival_time <- sig_feature_data[which(rownames(sig_feature_data) == training_neighbor), ]
        training_survival_time <- training_survival_time$time
        training_single_case_information[[1]] <- append(training_single_case_information[[1]], training_neighbor)
        training_single_case_information[[2]] <- append(training_single_case_information[[2]], training_survival_time)
        training_prognostic_score <- prognostic_scores[which(rownames(prognostic_scores) == training_neighbor), ]
        training_single_case_information[[3]] <- append(training_single_case_information[[3]], training_prognostic_score)
        training_risk_group <- risk_groups[which(rownames(risk_groups) == training_neighbor), ]
        training_single_case_information[[4]] <- append(training_single_case_information[[4]], training_risk_group)
        if(isEmpty(confidence)) {
          confidence <- 0
        }
        training_current_confidence <- training_current_confidence + confidence
        if(training_current_confidence > 1.0 && ! isEmpty(training_current_confidence)) {
          break
        }
        training_neighbor_times[which(rownames(training_neighbor_times) == training_query_case), 1] <- mean(unlist(training_single_case_information$`survival times`))
        training_neighbor_times[which(rownames(training_neighbor_times) == training_query_case), 3] <- prognostic_scores[which(rownames(prognostic_scores) == training_query_case),]

      }
      training_fold_case_information[[k]] <- training_single_case_information
      names(training_fold_case_information)[[k]] <- training_query_case
      training_number_retrieved[which(rownames(training_number_retrieved) == training_query_case),] <- length(training_single_case_information[[1]])
    }
    training_neighbor_times$`risk group` <- risk_groups[match(rownames(training_fold), rownames(risk_groups)), ]
    training_statistics <- suppressWarnings(coxph(Surv(training_neighbor_times$time, training_neighbor_times$status) ~ training_neighbor_times$`risk group`, data = training_features))



    training_sample_information[[i]] <- training_neighbor_times

    #training_dataframe <- data.table::rbindlist(lapply(training_sample_information, setDT, keep.rownames = TRUE))
    #training_dataframe <- column_to_rownames(training_dataframe, loc = 1)
