...
#' @param Num The number of training sets to create
...
CBRCon_Surv <- function(dataset, Num = 10, Percent = 0.5, Cores = 2, blocksize = 4000, Risk_Group_Num = 3, n.folds = 10, KM.Plot = TRUE) {
  set.seed <- 25
  ...
  if(KM.Plot == TRUE) {
    cat("-- Writing KM Plot for all samples (where survival time is real survival time) -- \n")
    ...
    png(filename = paste("Overall_KM_Plot.png"), width = 7.5, height = 7.5, units = "in", res = 300, pointsize = 7)
    ...
    cat("-- Writing KM Plot for all of the test cases (where survival time is mean of retrieved neighbor's survival times) -- \n")
    ...
    png(filename = paste("Testing_KM_Plot.png"), width = 7.5, height = 7.5, units = "in", res = 300, pointsize = 7)
    plot(fit, mark.time=TRUE, xlab = "Months", ylab = "Survival", main=paste("Survival Curve",sep=""),lty = 1:3, col = 1:3, cex = 0.5)
    grid()
    legend(x = "bottomright", legend = c(leg1, leg2, leg3), lty = 1:3,
           col = 1:3, cex = 0.65)
