...
#' Random Survival Forest Module
#'
#' This module will use random forest survival to predict survival.
#'
#'
...
coxlasso <- function(dataset, n.folds = 10, blocksize = 1000, Cores = 7) {
  ...
  for(i in 1:n.folds) {
    ...
    cat(" -- Running LASSO for fold ",i," -- \n")

    y = Surv(training_neighbor_times$time, training_neighbor_times$status)
    x = model.matrix(y ~ training_fold$riskgroup, training_features)
    training_lasso <- cv.glmnet(x, y, family = "cox", maxit = 5000)
    #training_lasso <- glmnet(x, y, family = "cox")


    test.y = Surv(testing_neighbor_times$time, testing_neighbor_times$status)
    test.x = model.matrix(test.y ~ testing_fold$riskgroup, testing_features)
    testing_lasso <- predict(training_lasso, newx = test.x, type = "response")
    testing_concordance <- apply(testing_lasso, 2, Cindex, y = test.y)

    concordance_values[i] <- testing_concordance
  }
